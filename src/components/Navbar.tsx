...
export function Navbar() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-sm border-b border-border/40 transition-shadow ${
        scrolled ? "shadow-sm" : ""
      }`}
    >
      <nav className="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        >
          <div className="h-10 w-10 rounded-full overflow-hidden relative bg-background/30">
            <Image
        ...
        <div className="hidden md:flex items-center space-x-8">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`relative transition-colors after:content-[''] after:absolute after:-bottom-1 after:left-0 after:h-0.5 after:bg-primary after:w-0 hover:after:w-full after:transition-all after:duration-200 ${
        ...
        {/* Mobile theme toggle (replaces hamburger) */}
        <button
          onClick={toggleTheme}
