      ...
      <section className={`py-24 transition-colors duration-300 ${isDark ? 'bg-[#141414]' : 'bg-gray-50'}`}>
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: 'Fresh Ingredients', description: 'We source locally and use only the freshest organic produce.', icon: '🥬', image: 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=400&q=80' },
              { title: 'Expert Chefs', description: 'Our chefs create nutritious meals that taste incredible.', icon: '👨‍🍳', image: 'https://images.unsplash.com/photo-1556910103-1c02745aae4d?w=400&q=80' },
              { title: 'Wellness Focus', description: 'Every dish is designed to support your health goals.', icon: '💚', image: 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=400&q=80' },
            ].map((feature, index) => (
          ...
          <div className="text-center max-w-3xl mx-auto mb-16">
            <InView animation="fade-up">
              <h2 className={`section-title mb-4`}>
                Our <span className="text-emerald-600 dark:text-emerald-400">Menu</span>
              </h2>
            </InView>
            <InView animation="fade-up" delay={100}>
            ...
            <div key={category} className="mb-12 last:mb-0">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {CAFE_MENU.filter(item => item.category === category).map((item, index) => (
                  <InView key={item.name} animation="fade-up" delay={index * 50}>
                    <div className={`rounded-xl p-5 transition-all duration-300 ${
                      ...
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <span className="text-emerald-600 dark:text-emerald-400 font-semibold whitespace-nowrap">{item.price}</span>
                      </div>
                    </div>
                  </InView>
                ))}
