    ...
    <div className="min-h-screen">
      ...
      <section className="relative py-32 sm:py-40 overflow-hidden bg-black">
        <div className="absolute inset-0">
          <Image
            src="/images/gallery/gym-hero.jpg"
      ...
      <section className={`py-24 transition-colors duration-300 ${isDark ? 'bg-[#141414]' : 'bg-gray-50'}`}>
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { title: 'Modern Equipment', description: 'Latest fitness machines and free weights from top brands.', icon: '🏋️', image: '/images/gallery/gym-equipment.jpg' },
              { title: 'Expert Trainers', description: 'Certified professionals to guide your fitness journey.', icon: '💪', image: '/images/gallery/gym-trainer.jpg' },
              { title: 'Group Classes', description: 'High-energy classes from yoga to HIIT training.', icon: '🧘', image: '/images/gallery/gym-classes.jpg' },
              { title: 'Nutrition Plans', description: 'Personalized diet plans to complement your training.', icon: '🥗', image: '/images/gallery/cafe-food.jpg' },
            ].map((feature, index) => (
              <InView key={feature.title} animation="fade-in" delay={index * 100}>
                <div className={`group rounded-2xl overflow-hidden transition-all duration-500 ${
                  ...
                  <div className="relative h-40 overflow-hidden">
                    <Image src={feature.image} alt={feature.title} fill className="object-cover transition-transform duration-700 group-hover:scale-110" sizes="(max-width: 768px) 100vw, 25vw" />
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <InView animation="fade-in">
              <h2 className={`section-title mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                Daily <span className="text-emerald-600">Classes</span>
              </h2>
            </InView>
            <InView animation="fade-in" delay={100}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {GYM_CLASSES.map((cls, index) => (
              <InView key={cls.name} animation="fade-in" delay={index * 100}>
                <div className={`rounded-xl p-5 transition-all duration-300 ${
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`text-xs px-2.5 py-1 rounded-full ${isDark ? 'bg-emerald-500/20 text-emerald-400' : 'bg-emerald-100 text-emerald-700'}`}>
                      {cls.duration}
                    </span>
                  </div>
